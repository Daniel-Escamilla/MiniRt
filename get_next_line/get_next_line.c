...
/*   Updated: 2025/02/14 18:56:58 by descamil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*read_l(int fd, char *buffer)
{
	...
	while (1)
	{
		if (buffer && buffer[0] == '\0')
			buffer = NULL;
		bytes_read = (int)read(fd, aux, BUFFER_SIZE);
		if (bytes_read < 0)
		{
			free(buffer);
			return (NULL);
		...
		if (!buffer)
			return (NULL);
		if (bytes_read != BUFFER_SIZE || ft_strchr_b(buffer, '\n'))
			return (buffer);
	}
	return (NULL);
}
...
static char	*delete_line(char *buffer)
{
	...
	i++;
	new = ft_calloc((size_t)(ft_strlen_b(buffer) - i) + 1, 1);
	if (!new)
		return (NULL);
	j = 0;
	while (buffer[i])
